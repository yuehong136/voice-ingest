...
显式删除留下不可重试的 `result_deleted` 幂等墓碑，并提升 generation。转写导出按需渲染，
避免和删除竞争时重新写回对象。存储删除失败需重放 DELETE；没有自动物理清理保证。
发布审查、停写备份与恢复边界见 [部署演练](../operations/deployment-rehearsal.md)。
