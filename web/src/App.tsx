...
import { SynthesisWorkspace } from './synthesis/SynthesisWorkspace'
...
export default function App() {
  ...
  const [pane, setPane] = useState<'transcription' | 'synthesis'>('transcription')
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label={t('Workspace', '工作区')}>
          <button
            className={`nav-item ${pane === 'transcription' ? 'active' : ''}`}
            onClick={() => setPane('transcription')}
          >
            <FolderOpen size={19} />
            {t('Recordings', '录音工作台')}
          ...
          <button
            className={`nav-item ${pane === 'synthesis' ? 'active' : ''}`}
            onClick={() => setPane('synthesis')}
          >
            <AudioLines size={19} />
            {t('Speech synthesis', '语音合成')}
          </button>
      ...
      <main>
        <header className="topbar">
          <div className="breadcrumb">
            ...
            <span>/</span>
            <strong>
              {pane === 'synthesis' ? t('Speech synthesis', '语音合成') : t('Recordings', '录音')}
            </strong>
          </div>
        ...
        <nav className="capability-tabs" aria-label={t('Speech capabilities', '语音能力')}>
          <button aria-pressed={pane === 'transcription'} onClick={() => setPane('transcription')}>
            {t('Transcriptions', '转写')}
          </button>
          <button aria-pressed={pane === 'synthesis'} onClick={() => setPane('synthesis')}>
            {t('Speech synthesis', '语音合成')}
          </button>
        </nav>
        {pane === 'synthesis' ? (
          ...
          <div className="workspace-content">
            <div className="page-heading">
              ...
              <button
                ...
                disabled={!!api && !models.data?.some((m) => m.kind === 'transcription')}
              >
                <Plus size={19} />
                                  ...
                                  onClick={() => {
                                    if (audio.current && segment.start_ms != null) {
                                      ...
                                      void audio.current
                                        ...
                                        .catch(() =>
                                          setNotice(
                                            t('Press play to listen.', '点击播放即可回听。'),
                                          ),
                                        )
                                    }
                                  }}
            ...
            <footer className="page-footer">
              ...
              <span>
                HTTP API <span className="dot">/</span> Python SDK <span className="dot">/</span>{' '}
                MCP
              </span>
            </footer>
          </div>
      ...
      {dialog === 'upload' && api && (
        <UploadModal
          api={api}
          models={(models.data || demoModels).filter((m) => m.kind === 'transcription')}
          draft={draft}
          prepared={setDraft}
          zh={zh}
